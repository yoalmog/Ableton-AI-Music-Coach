...
export const NewHeader: React.FC<NewHeaderProps> = ({
  ...
  return (
    <header
      dir={isRTL ? 'rtl' : 'ltr'}
      className="bg-[#0E0E0E] border-b border-[#242424] text-[#E0E0E0] select-none z-30 font-sans shrink-0 w-full max-w-full flex flex-col shadow-lg"
    >
      {/* ========================================================================= */}
      {/* ROW 1: PRIMARY STUDIO CONTROL BAR (~50px)                                */}
      {/* ========================================================================= */}
      <div className="h-[50px] px-3 md:px-4 bg-[#111111] flex items-center justify-between gap-3 min-w-0 border-b border-[#202020]">
        ...
        {/* Right Section: Interactive DAW Utility Actions */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-[#181818] hover:bg-[#242424] text-gray-300 hover:text-white border border-[#2D2D2D] hover:border-[#404040] transition-colors cursor-pointer text-xs font-mono"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-[#132213] hover:bg-[#1A301A] border border-[#224822] text-[#90FF00] transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer text-xs font-mono font-bold shadow-sm"
          >
            <span
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1.5 rounded-md bg-[#181818] hover:bg-[#242424] text-gray-300 hover:text-white border border-[#2D2D2D] transition-colors cursor-pointer text-xs font-mono font-bold"
            >
            ...
            {isLangMenuOpen && (
              <div
                className="absolute top-full mt-1.5 right-0 rtl:right-auto rtl:left-0 z-50 w-36 bg-[#161616] border border-[#303030] rounded-lg shadow-2xl p-1 text-xs font-mono animate-in fade-in zoom-in-95 duration-100"
                dir={isRTL ? 'rtl' : 'ltr'}
              >
                {supportedLanguages.map((langConfig) => (
                  ...
                  >
                    ...
                    {language === langConfig.code && <Check className="w-3.5 h-3.5" />}
                  </button>
                ))}
              </div>
            )}
      ...
      {/* ========================================================================= */}
      {/* ROW 2: COMPACT DAW PROJECT STATUS BAR (~38px)                             */}
      {/* ========================================================================= */}
      <div
        dir={isRTL ? 'rtl' : 'ltr'}
        className="h-[38px] px-3 md:px-4 bg-[#141414] border-t border-[#202020] flex items-center justify-between text-xs font-mono text-[#E0E0E0] select-none overflow-x-auto no-scrollbar gap-2"
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {saveStatus && (
            <span className="text-[10px] text-[#90FF00] bg-[#162516] border border-[#234523] px-2 py-0.5 rounded font-bold animate-pulse">
              {saveStatus}
            </span>
          )}
        </div>

        {/* Center Block: Musical Parameters (Technical Values Strictly Formatted in LTR) */}
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 text-[11px]">
          {/* Genre Badge */}
          <div className="hidden sm:flex items-center gap-1.5 bg-[#1A1A1A] border border-[#282828] px-2 py-0.5 rounded">
            <span className="text-gray-400 font-bold">{t('header.genre')}</span>
            <span className="text-[#FFB703] font-bold truncate max-w-[100px]" dir="ltr">
              {project.genre || 'Psytrance'}
            </span>
          </div>

          {/* Key & Scale Badge */}
          <div className="flex items-center gap-1.5 bg-[#1A1A1A] border border-[#282828] px-2 py-0.5 rounded">
          ...
          {/* Time Signature / Grid (Extended Large Screen DAW Info) */}
          <div className="hidden xl:flex items-center gap-1 bg-[#1A1A1A] border border-[#282828] px-2 py-0.5 rounded text-gray-400">
            <span className="text-gray-500 font-bold">GRID:</span>
            <span className="text-gray-300 font-bold" dir="ltr">
              4/4 • 1/16
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0 text-[11px]">
          ...
          <button
            ...
            title={t('header.aiStatusTooltip')}
            className="flex items-center gap-1.5 bg-[#1A1A1A] hover:bg-[#252525] border border-[#282828] px-2 py-0.5 rounded transition cursor-pointer"
          >
          ...
          <div className="hidden lg:flex items-center gap-1.5 bg-[#1A1A1A] border border-[#282828] px-2 py-0.5 rounded text-gray-300">
            <span className="text-gray-400 font-bold">{t('header.abletonStatus')}:</span>
            <span className="font-bold text-[#90FF00]" dir="ltr">
              Live 12 Link
            </span>
          </div>
        </div>
